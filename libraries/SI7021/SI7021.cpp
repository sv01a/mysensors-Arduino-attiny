...
#else
    if (!rtc.transfer(I2C_ADDR | I2C_WRITE, reg, reglen, I2C_CONTINUE))
    {
        return;
    }
    // Write data.
    rtc.transferContinue(reg, reglen);
#endif
}
...
int SI7021::_readReg(byte *reg, int reglen)
{
    ...
    if (!rtc.transfer(I2C_ADDR | I2C_WRITE, 0, 1))
        return false;

    // Read data.
    return rtc.transfer(I2C_ADDR | I2C_READ, reg, reglen);
#endif
}
